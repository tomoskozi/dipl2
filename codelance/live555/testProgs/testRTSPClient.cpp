...
void initOpts(cOpts *pOpts)
{
    pOpts->output       = "delay.csv";
    pOpts->dropRate     = 1000000000U;
    pOpts->repeatCnt    = 5U;
    pOpts->srcAddr      = "192.0.0.1";
    pOpts->dstAddr      = "192.0.0.1";
} /* initOpts  */

void parseOpts(int argc, char** argv, cOpts *pOpts)
{
    char *optStr = "o:d:r:p:q:hsfa";
    int opt = -1;
    while((opt = getopt(argc, argv, optStr)) != -1) 
    {
        ...
        case 'o':
            ...
            break;
        case 'd':
            pOpts->dropRate         = atoi(optarg);
            break;
        case 'r':
            pOpts->repeatCnt        = atoi(optarg);
            break;
        case 's':
            pOpts->systematicOff    = true;
            printf("sysOff\n");
            break;
        case 'f':
            pOpts->fbOff    = true;
            ...
            break;
        case 'a':
            pOpts->repeatAfterOver  = true;
            printf("repeatAfterOver\n");
            break;
        case 'p':
            pOpts->srcAddr  = optarg;
            break;
        case 'q':
            pOpts->dstAddr  = optarg;
            break;
        default:
        case 'h':
            printf("usage: \n");
...
//  for (int i = 1; i <= argc-1; ++i) {
    ...
    printf("%s\n", Opts::opts.srcAddr);
     openURL(*env, argv[0], Opts::opts.srcAddr);
//  }
...
void continueAfterSETUP(RTSPClient* rtspClient, int resultCode, char* resultString) {
  do {
    ...
    //scs.subsession->sink = MultiFramedRTPSource::createNew(env, *scs.subsession, rtspClient->url());
...
void DummySink::afterGettingFrame(unsigned frameSize, unsigned numTruncatedBytes,
  ...
  write_packet_dumpx(fReceiveBuffer, frameSize);
