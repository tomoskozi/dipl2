...
void MultiFramedRTPSink::setPacketSizes(unsigned preferredPacketSize,
  ...
  fEncBuf = new OutPacketBuffer(preferredPacketSize, maxPacketSize);
...
MultiFramedRTPSink::MultiFramedRTPSink(UsageEnvironment& env,
  ...
  : RTPSink(env, rtpGS, rtpPayloadType, rtpTimestampFrequency,
    ...
    fOutBuf(NULL), fEncBuf(NULL), pNCSink(codelance::NCRTPSink::Create()), fCurFragmentationOffset(0), fPreviousFrameEndedFragmentation(False),
    fOnSendErrorFunc(NULL), fOnSendErrorData(NULL) {
  setPacketSizes(1000, /*1448*/1200);
    //this->pNCSink = new codelance::NCRTPSink(/*32, 1300*/);
...
#ifdef PERIODICAL_DROPS
  static size_t timer = 0U;
#else
#define timer our_random()
//#define timer rand()
    //srand(time(NULL));
#endif
  if (fNumFramesUsedSoFar > 0) {
...
#endif
	/*OutPacketBuffer nc(sizeof (g_rtp_pkt), 10000);
	nc.enqueue(g_rtp_pkt, sizeof (g_rtp_pkt));*/
	//fEncBuf->enqueue(g_rtp_pkt, sizeof (g_rtp_pkt));
	/*fOutBuf = &nc;*/
	/*fOutBuf->resetPacketStart();
	fOutBuf->resetOffset();
	fOutBuf->enqueueNC(g_rtp_pkt, sizeof (g_rtp_pkt));*/
	//write_packet_dump(fOutBuf/*fEncBuf*/->packet() + 12, fOutBuf/*fEncBuf*/->curPacketSize() - 12);

    static const size_t DROP_RATE = Opts::opts.dropRate;//5;//50;//10000000;//10;//100U;
    ...
    static FILE *pDump = fopen("encoding.csv", "w");

    fprintf(pDump, "%u;%u;%lld\n",
        0, fSeqNo - 1, td.total_milliseconds());
#else
    this->pNCSink->PushPacket(fOutBuf->packet(), fOutBuf->curPacketSize());
    ...
    //timer = 1;
    // refresh parameters
#ifdef RETRANSMIT_PARAMS
    if ((timer % 200) == 0)
    {
        this->pNCSink->SendParameters();
    }
#endif

    /*void *ptr = 0;
    size_t sz = 0U;
    this->pNCSink->PopCode(&ptr, &sz);
	fEncBuf->enqueue(ptr, sz);*/
   

    while (this->pNCSink->IsNextNew())
    {
         // for next time

        fEncBuf->resetPacketStart();
        fEncBuf->resetOffset(); 

        if (!this->pNCSink->PopCode(this->fEncBuf))
        {
            break;
        }

	    //write_packet_dump(/*fOutBuf*/fEncBuf->packet(), /*fOutBuf*/fEncBuf->curPacketSize());
        //printf("%u ? %u\n", timer, (timer % DROP_RATE) != 0U ? 0 : 1);
        if ( ((timer % DROP_RATE) != 0U) 
            && !fRTPInterface.sendPacket(/*fOutBuf*/fEncBuf->packet(), /*fOutBuf*/fEncBuf->curPacketSize())) 
        {
	        // if failure handler has been specified, call it
	        if (fOnSendErrorFunc != NULL) (*fOnSendErrorFunc)(fOnSendErrorData);
        }
    ...
    // redundancy
    if (this->pNCSink->IsComplete())
    {
        this->pNCSink->Repeat(Opts::opts.repeatCnt/*5U*/);
    }
    for (size_t i = 0; i < this->pNCSink->packets.size(); ++i)
    {
        auto p = &(this->pNCSink->packets[i]);
        ...
        if (p->second == true)
        {
            ...
            fEncBuf->resetPacketStart();
            fEncBuf->resetOffset();
            fEncBuf->enqueue(&(p->first[0]), p->first.size());
            //write_packet_dump(fEncBuf->packet(), fEncBuf->curPacketSize());
            ...
            if ((timer % DROP_RATE) != 0U)
            {
                fRTPInterface.sendPacket(fEncBuf->packet(), fEncBuf->curPacketSize());
            }
...
#if 0
    for (auto p : this->pNCSink->packets)
    {
        if (p.second == true)
        {
            p.second = false;
...
#endif
    /*while (this->pNCSink->packets.size() > 0)
    {*/
        /*auto p = this->pNCSink->packets.front();

        write_packet_dump(&(p[0]), p.size());

        fEncBuf->resetPacketStart();
        fEncBuf->resetOffset();
        fEncBuf->enqueue(&(p[0]), p.size());
        //write_packet_dump(fEncBuf->packet(), fEncBuf->curPacketSize());

        fRTPInterface.sendPacket(fEncBuf->packet(), fEncBuf->curPacketSize());
        this->pNCSink->packets.pop_front();*/
    //}
#if 0
        /*auto packets = this->pNCSink->Repeat(5U);
        for (auto p : packets)
        {
            fEncBuf->resetPacketStart();
	        fEncBuf->resetOffset();
        ...
        for (int i = 0; i < 5; ++i)
        {
            ...
            this->pNCSink->PopCode(this->fEncBuf);
	        //write_packet_dump(fEncBuf->packet(), fEncBuf->curPacketSize());
